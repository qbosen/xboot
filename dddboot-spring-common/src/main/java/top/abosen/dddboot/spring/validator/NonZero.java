...
public @interface NonZero {
    ...
    class NotZeroIntegerValidator implements ConstraintValidator<NonZero, Integer> {
        @Override public boolean isValid(Integer num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroLongValidator implements ConstraintValidator<NonZero, Long> {
        @Override public boolean isValid(Long num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroShortValidator implements ConstraintValidator<NonZero, Short> {
        @Override public boolean isValid(Short num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroFloatValidator implements ConstraintValidator<NonZero, Float> {
        @Override public boolean isValid(Float num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroDoubleValidator implements ConstraintValidator<NonZero, Double> {
        @Override public boolean isValid(Double num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroByteValidator implements ConstraintValidator<NonZero, Byte> {
        @Override public boolean isValid(Byte num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }
    ...
    class NotZeroBigIntegerValidator implements ConstraintValidator<NonZero, BigInteger> {
        @Override public boolean isValid(BigInteger num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }

    class NotZeroBigDecimalValidator implements ConstraintValidator<NonZero, BigDecimal> {
        @Override public boolean isValid(BigDecimal num, ConstraintValidatorContext constraintValidatorContext) {
            return num == null || NumberSignHelper.signum(num) != 0;
        }
    }
}
